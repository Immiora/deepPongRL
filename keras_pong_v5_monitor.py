...
# export PATH="/vol/optdcc/anaconda2/bin:$PATH"
# cd /vol/ccnlab-scratch1/julber
# python ./RL/Project/keras_pong_v4.py -b 50 -m mlp -l 2 -d 100 -o ./RL/Project/dense2_100_batch_size50/
...
def learning_model(args, learning_rate, input_dim=80*80, number_of_inputs = 2):
  ...
  #    opt = RMSprop(lr=learning_rate)
      ...
      model.add(Conv2D(filters = args.nfilters1, kernel_size = args.size_filters1))
      # model.add(BatchNormalization())
      model.add(Activation('relu'))
      model.add(Dropout(args.regularizer))
      if args.nlayers > 1:
        model.add(Conv2D(filters = args.nfilters1, padding = 'same', kernel_size= args.size_filters1))
        # model.add(BatchNormalization())
        model.add(Activation('relu'))
      model.add(Flatten())
      model.add(Dense(args.ndense))
...
def main():
  ...
  parser.add_argument('--size_filters2', '-s2', type = int, default=6,
                      help='Size of conv filters')
  parser.add_argument('--regularizer', '-r', type=float, default=0.,
  ...
  # ##
  # filename = 'pong_model_checkpoint.h5'
